...
RSpec.describe AccountStatusesCleanupPolicy do
  ...
  describe '#statuses_to_delete' do
    ...
    let!(:faved_4_count)            { Fabricate(:status, created_at: 1.year.ago, account: account) }
    let!(:faved_5_count)            { Fabricate(:status, created_at: 1.year.ago, account: account) }
    let!(:reblogged_4_count)        { Fabricate(:status, created_at: 1.year.ago, account: account) }
    let!(:reblogged_5_count)        { Fabricate(:status, created_at: 1.year.ago, account: account) }
    let!(:recent_status)     { Fabricate(:status, created_at: 2.days.ago, account: account) }
    ...
    before do
      4.times { faved_4_count.increment_count!(:favourites_count) }
      5.times { faved_5_count.increment_count!(:favourites_count) }
      4.times { reblogged_4_count.increment_count!(:reblogs_count) }
      5.times { reblogged_5_count.increment_count!(:reblogs_count) }
    end
    ...
    context 'when policy is set to keep DMs and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(very_old_status.id, pinned_status.id, self_faved.id, self_bookmarked.id, status_with_poll.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep self-bookmarked toots and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, pinned_status.id, self_faved.id, status_with_poll.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep self-faved toots and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, pinned_status.id, self_bookmarked.id, status_with_poll.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep toots with media and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, pinned_status.id, self_faved.id, self_bookmarked.id, status_with_poll.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep toots with polls and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, pinned_status.id, self_faved.id, self_bookmarked.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep pinned toots and reject everything else' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, self_faved.id, self_bookmarked.id, status_with_poll.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is to not keep any special messages' do
      ...
      it 'returns every other old status for deletion' do
        expect(subject.pluck(:id)).to include(direct_message.id, very_old_status.id, pinned_status.id, self_faved.id, self_bookmarked.id, status_with_poll.id, status_with_media.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is set to keep every category of toots' do
      ...
      it 'returns only normal statuses for deletion' do
        expect(subject.pluck(:id)).to contain_exactly(very_old_status.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
    ...
    context 'when policy is to keep statuses with at least 5 boosts' do
      ...
      it 'does not return the toot reblogged 5 times' do
        expect(subject.pluck(:id)).to_not include(reblogged_5_count.id)
      end
      ...
      it 'returns old statuses not reblogged as much' do
        expect(subject.pluck(:id)).to include(very_old_status.id, faved_4_count.id, faved_5_count.id, reblogged_4_count.id)
      end
    end
    ...
    context 'when policy is to keep statuses with at least 5 favs' do
      ...
      it 'does not return the toot faved 5 times' do
        expect(subject.pluck(:id)).to_not include(faved_5_count.id)
      end
      ...
      it 'returns old statuses not faved as much' do
        expect(subject.pluck(:id)).to include(very_old_status.id, faved_4_count.id, reblogged_4_count.id, reblogged_5_count.id)
      end
    end
  end
end
